...
function keys() {
    ...
    return {publicKey:  encodeBase64(keyPair.publicKey), privateKey: encodeBase64(keyPair.secretKey)}
}
...
test('PlayerID not registered', () => {
    ...
    const player1Metadata: Client['metadata'] & {playerID: string} = {playerID: "Player1", credentials: encodeBase64(player1keyPair.publicKey), message: undefined }
...
test('PlayerID null', () => {
    const player1keyPair =  nacl.sign.keyPair()
    const player1Metadata: Client['metadata'] = {playerID: null, credentials: encodeBase64(player1keyPair.publicKey), message: undefined}
